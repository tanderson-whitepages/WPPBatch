...
for row in csvReader:
	...
	if rowNum == 1:
		...
		headers.append('Other Current Addresses')
		...
		headers.append('Historical Addresses')
		headers.append('Persons Associated')
		headers.append('Alternate Phones')
		...
		for owner in belongsTo:
			...
			location = currAddresses[0]
			otherCurrAddress = ''
			if len(currAddresses) > 1:
				l = currAddresses[1]
				otherCurrAddress += str(l.get('street_line_1',''))+' '+str(l.get('city',''))+' '+str(l.get('state_code',''))+' '+str(l.get('postal_code','')) + '; '
			
			addressType = location.get('location_type','')
			...
			decodedRow = []
			for hist in histAddresses:

				if histAddresses == [{}]:
					histStreet = ''
				else:
					histStreet += str(hist.get('street_line_1',''))+' '+str(hist.get('city',''))+', '+str(hist.get('state_code',''))+' '+str(hist.get('postal_code','')) + '; '
				
			#	decodedRow = []
			
			resultRow.append(histStreet)

			for person in associatedPeople:
			
				assPeople += wppbatchlib.nvl(person.get('name',''),'')+' - '+wppbatchlib.nvl(person.get('relation',''),'')+"; "
				
			resultRow.append(assPeople)	
			
			for phone in altPhones:
