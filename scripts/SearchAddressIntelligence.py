...
###################################################################################################
#  SEARCH ADDRESS
#  This script runs Whitepages Pro API reverse address queries based on an input CSV file.
...
#  ExtractAddress.bat to parse that JSON and generate the final results file.
###################################################################################################
...
try:	
	...
	while not doneWithFile:
		...
		if row != 'thisistheend':
			numInputs += 1
			#build URL
			apiURL = 'http://'+apiHost+'/3.0/location_intel.json?'
			for i in inputMap:
